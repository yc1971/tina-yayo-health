    ...
    <div className="glass rounded-xl p-4 md:p-5">
      <div className="flex items-start justify-between">
        <div className="min-w-0 flex-1">
          <div className="text-muted-foreground text-[10px] md:text-xs font-medium uppercase tracking-wider mb-1.5 md:mb-2">{label}</div>
          <div className="text-2xl md:text-3xl font-bold text-foreground mono">{value}</div>
          {sub && <div className="text-[10px] md:text-xs text-muted-foreground mt-1 leading-tight">{sub}</div>}
        </div>
        <div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center flex-shrink-0 ml-2 ${color}`}>
...
function StatusBadge({ status }: { status: string | null | undefined }) {
  ...
  const labels: Record<string, string> = {
    normal: "Normal", high: "Alto", low: "Bajo", critical: "Crítico",
  };
  const icons: Record<string, any> = {
    high: TrendingUp, low: TrendingDown, normal: Minus, critical: AlertTriangle,
  };
    ...
    <div className="p-4 md:p-8 space-y-5 md:space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 md:gap-3 flex-wrap">
          ...
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium border ${patientBadge}`}>
            {patientAge}
          </span>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {statsLoading ? (
          ...
          <>
            <StatCard label="Estudios" value={totalStudies} icon={FileText} color="bg-primary/10 text-primary" />
            <StatCard label="Laboratorios" value={totalLabs} icon={FlaskConical} color="bg-blue-500/10 text-blue-400" />
            <StatCard label="Imágenes" value={totalScans} icon={Scan} color="bg-violet-500/10 text-violet-400" sub={`+ ${totalXrays} Rx`} />
            <StatCard
              label="Alertas"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        {/* Recent studies — full width mobile, 2/3 desktop */}
                ...
                return (
                  <Link key={study.id} href={`/studies/${study.id}`}>
                    <div
                      ...
                      className="flex items-center gap-3 p-3 rounded-lg hover:bg-secondary/50 active:bg-secondary cursor-pointer transition-colors group"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {new Date(study.studyDate).toLocaleDateString("es-PR")}
                        </div>
                      </div>
                      <Badge variant="outline" className="text-xs flex-shrink-0 hidden sm:inline-flex">
                        {getCategoryLabel(study.category)}
                      </Badge>
                      <ChevronRight className="w-4 h-4 text-muted-foreground opacity-40 md:opacity-0 md:group-hover:opacity-100 transition-opacity flex-shrink-0" />
                    </div>
                  </Link>
                );
              })}
              ...
              {activeAlerts.map(alert => (
                <div
                  ...
                  className={`p-3 rounded-lg border ${alert.type === "critical"
                    ? "glow-critical border-red-500/20 bg-red-500/5"
                    : "glow-warning border-amber-500/20 bg-amber-500/5"}`}
                >
                  <div className="flex items-start gap-2">
                    <AlertTriangle className={`w-4 h-4 mt-0.5 flex-shrink-0 ${alert.type === "critical" ? "text-red-400" : "text-amber-400"}`} />
                    <div className="min-w-0">
                      <div className="text-xs font-medium text-foreground leading-tight">{alert.title}</div>
                      <div className="text-[11px] text-muted-foreground mt-0.5 line-clamp-2 leading-snug">{alert.description}</div>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* Lab markers out of range — 2 cols mobile, 4 cols desktop */}
      {studies.filter(s => s.category === "lab").length > 0 && (
        <div className="glass rounded-xl p-4 md:p-6">
          <h2 className="text-sm font-semibold text-foreground mb-3 md:mb-4">
            Marcadores Fuera de Rango — {patientLabel}
          </h2>
            ...
            {(stats?.labResults ?? [])
              ...
              .map((r: any, i: number) => (
                <div key={i} className="p-3 md:p-4 rounded-lg bg-secondary/50 border border-border">
                  <div className="text-[10px] md:text-xs text-muted-foreground mb-1 truncate">{r.markerName}</div>
                  <div className="font-semibold text-foreground mono text-base md:text-lg">
                    {r.value} <span className="text-[10px] md:text-xs font-normal text-muted-foreground">{r.unit}</span>
                  </div>
